          ...
          <div v-if="activeTab === 'property-overview'" class="">
                ...
                <table data-aos="fade-up"  class="w-full mt-2 table-fixed text-sm">
                  ...
                  <tbody>
                    <tr v-for="(room, index) in formattedRoomData" :key="index">
                      <td class="text-[#1D2739] font-medium border-b-[0.5px] border-gray-50 py-6 pl-6">{{ room.occupant }}</td>
                      <td class="text-[#1D2739] font-medium border-b-[0.5px] border-gray-50 py-6">{{ room.roomOccupied }}</td>
                      <td class="text-[#1D2739] font-medium border-b-[0.5px] border-gray-50 py-6">{{ room.availableFrom }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <section class="mt-10">
              ...
              <div v-else
                ...
                <img :src="dynamicIcons('payment-empty-state')" alt="" class="" />
                <!-- {{ availabilityList }} -->
        ...
        <div data-aos="fade-up"  class="bg-[#F0F2F5] p-6 rounded-md">
          <div class="flex items-center justify-between space-x-4">
            ...
            <div class="ml-auto flex space-x-2">
              <button disabled :disabled="!property?.agent?.phoneNumber"
                @click="copyToClipboard(property?.agent?.phoneNumber)"
                class="p-2 disabled:cursor-not-allowed disabled:opacity-50 rounded-full">
          ...
          <div v-if="showApplicationBtn" class="mt-10">
            <!-- <button :disabled="property?.rentalApplication?.progress === 'APPROVED'"
   ...
   <div class="flex items-center gap-x-3">
    ...
    <CoreModal :isOpen="openRentalApplicationModal">
      <div class="max-w-xl mx-auto bg-white rounded-md">
        ...
        <div class="pt-10">
          <button @click="handleSelectedRoom" class="w-full p-3.5 text-sm rounded-lg"
            :class="selectedRoom ? 'bg-[#292929] text-white' : 'bg-gray-300 text-gray-500'" :disabled="!selectedRoom">
            Continue
          </button>
        </div>
      </div>
    ...
    <CoreModal :isOpen="showBookingModal" @close="showBookingModal = false">
